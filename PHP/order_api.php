...
try {
    echo "Debug: Starting order_api with action '{$action}'\n";

    switch ($action) {
        case 'list':
            echo "Debug: Handling list action\n";
        ...
        case 'list_all':
            echo "Debug: Handling list_all action\n";
        ...
        case 'create':
            echo "Debug: Handling create action\n";
            ...
            foreach ($items as $it) {
                ...
                if (!$inventory || $inventory['Stock_Quantity'] < $quantity) {
                    echo "Debug: Insufficient stock for product {$productId}\n";
            ...
            foreach ($items as $it) {
                ...
                if ($inventory && $inventory['Stock_Quantity'] < 20) {
                    echo "Debug: Low stock notification triggered for product {$productId}\n";
        ...
        case 'view':
            echo "Debug: Handling view action for order\n";
        ...
        default:
            echo "Debug: Invalid action encountered\n";
            sendJsonResponse(['error' => 'Invalid action'], 400);
    }
} catch (Throwable $e) {
    echo "Debug: Exception in order_api.php - " . $e->getMessage() . "\n";
