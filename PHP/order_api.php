...
switch ($action) {
    ...
    case 'mark_delivered':
        ...
        [$userId, $user] = resolveUserContext($pdo, $_POST, [
            'includeUser' => true,
            'notFoundMessage' => 'We could not verify your account for this order.',
        ]);

        $orderId = filter_input(INPUT_POST, 'order_id', FILTER_VALIDATE_INT);
        if (!$orderId) {
            sendJsonResponse(['error' => 'Invalid order ID provided.'], 400);
        }
        ...
        if ((int)($order['User_ID'] ?? 0) !== $userId) {
            sendJsonResponse(['error' => 'You do not have permission to update this order.'], 403);
        }
        ...
        $deliverableStatuses = ['Pending', 'Processing', 'Confirmed', 'Ready for pickup', 'Shipped', 'On Delivery'];
        if (!in_array($currentStatus, $deliverableStatuses, true)) {
            sendJsonResponse([
                'error' => 'This order cannot be marked as delivered in its current status.',
                'order' => [
                    'order_id' => $orderId,
        ...
        updateOrderStatus($pdo, $orderId, 'Delivered');

        record_audit_log($pdo, 'order_marked_delivered', "Order #{$orderId} marked as delivered by customer.", [
            'actor_id' => $userId ?: null,
            'actor_email' => $user['Email'] ?? null,
            'source' => 'order_api',
            'metadata' => [
